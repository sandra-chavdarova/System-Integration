...
public class Repository<T> : IRepository<T> where T : class
{
        ...
        bool asNoTracking = false)
    {
        ...
        if (typeof(E) == typeof(T)) return (Task<E?>)(object)finalQuery.FirstOrDefaultAsync();
    ...
    public async Task<PaginatedResult<E>> GetAllPagedAsync<E>(
        Expression<Func<T, E>> selector, int pageNumber, int pageSize,
        Expression<Func<T, bool>>? predicate = null,
        Func<IQueryable<T>, IOrderedQueryable<T>>? orderBy = null,
        Func<IQueryable<T>, IIncludableQueryable<T, object>>? include = null,
        bool asNoTracking = false)
    {
        ...
        if (orderBy != null)
            pagedQuery = orderBy(query).Skip(pageNumber * pageSize).Take(pageSize).Select(selector);
        else
            pagedQuery = query.Skip(pageNumber * pageSize).Take(pageSize).Select(selector);
