...
public class ExternalEnrollmentApiTests : LoggedTestBase, IClassFixture<WebApplicationFactory<Program>>, IAsyncLifetime
{
    ...
    public ExternalEnrollmentApiTests(WebApplicationFactory<Program> factory, GlobalTestFixture fixture) : base(fixture)
    {
        _factory = factory
            .WithTestDatabase()
    ...
    public async Task GetStatus_ValidId_ShouldReturnStatusObject()
    {
        await RunTestAsync(async () =>
        {
            ...
            Assert.True(postDoc.RootElement.TryGetProperty("id", out var idProp), "202 response must contain 'id' field");
            ...
            Assert.True(statusDoc.RootElement.TryGetProperty("status", out _), "Status response must contain 'status' field");
            Assert.True(statusDoc.RootElement.TryGetProperty("id", out _), "Status response must contain 'id' field");
        });
    }
